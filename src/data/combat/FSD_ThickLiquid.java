 ...
 */
public class FSD_ThickLiquid extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
        OnHitEffectPlugin,
        EveryFrameWeaponEffectPlugin {

    public FSD_ThickLiquid() {
    }

    protected List<data.combat.FSD_ThickLiquid> trails;
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        if (trails == null) return;

        Iterator<data.combat.FSD_ThickLiquid> iter = trails.iterator();
        while (iter.hasNext()) {
        ...
        if (weapon.getShip() != null) {
            ...
            for (data.combat.FSD_ThickLiquid curr : trails) {
                if (curr.proj != null) {
        ...
        for (int i = 0; i < numIter; i++) {
            for (data.combat.FSD_ThickLiquid trail : trails) {
...
//										rampUp, 0f, dur, inverted);
        ...
        if(target instanceof MissileAPI){
            projectile.setDamageAmount(projectile.getDamageAmount() * 1.5f);
        }
        if(target instanceof ShipAPI ){
            ShipAPI ship = (ShipAPI) target;
            if(ship.getHullSpec().getHullSize() == ShipAPI.HullSize.FIGHTER){
                projectile.setDamageAmount(projectile.getDamageAmount() * 1.5f);
            }
    ...
    public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
        ...
        com.fs.starfarer.api.impl.combat.CryofluxTransducerEffect trail = new com.fs.starfarer.api.impl.combat.CryofluxTransducerEffect(projectile, prev);
        ...
        if (trails == null) {
            trails = new ArrayList<data.combat.FSD_ThickLiquid>();
        }
        trails.add(0, new data.combat.FSD_ThickLiquid(projectile, prev));
    }
    ...
    public static class ParticleData {
        ...
        public ParticleData(DamagingProjectileAPI proj, FSD_ThickLiquid fsdThickLiquid) {
            this.proj = proj;
...
//			offset.y += vel.y * 1f;
    ...
    public FSD_ThickLiquid(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
        this.proj = proj;
        this.prev = prev;
