...
public class FSD_MiniHESH implements EveryFrameWeaponEffectPlugin {
...
//            if(weapon.getChargeLevel() == 1)weapon.getShip().getFluxTracker().increaseFlux(weapon.getFluxCostToFire()*(1/3f), false);
//        }
        if(!weapon.getShip().hasListenerOfClass(MiniHESH_RangeModifier.class)){
            weapon.getShip().addListener(new MiniHESH_RangeModifier(weapon,weapon.getShip().getHullSize()));
        }
...
//            }
    ...
    public static class MiniHESH_RangeModifier implements WeaponBaseRangeModifier {
        public WeaponAPI weapon;
        public ShipAPI.HullSize size;
        public MiniHESH_RangeModifier(WeaponAPI weapon,ShipAPI.HullSize size){
            this.weapon = weapon;
            this.size = size;
        }
        @Override
