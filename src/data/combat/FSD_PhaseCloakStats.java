...
public class FSD_PhaseCloakStats extends BaseShipSystemScript {
    ...
    public static float BREATH_SPEED = 1.0f;
    public static float BREATH_MIN_ALPHA = 0.1f;
    public static float BREATH_MAX_ALPHA = 1.0f;
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if (FLUX_LEVEL_AFFECTS_SPEED) {
            if (state == State.ACTIVE || state == State.OUT || state == State.IN) {
                ...
                breathPhase += BREATH_SPEED * Global.getCombatEngine().getElapsedInLastFrame();
                if (breathPhase > 1f) breathPhase -= 1f;
                float alpha = BREATH_MIN_ALPHA + (BREATH_MAX_ALPHA - BREATH_MIN_ALPHA) * Math.abs(0.75f - breathPhase) * 0.85f;
                if (mult < 1f) {
                    stats.getMaxSpeed().modifyMult(id + "_2", mult);
                } else {
                    stats.getMaxSpeed().unmodifyMult(id + "_2");
                }
                ((PhaseCloakSystemAPI)cloak).setMinCoilJitterLevel(alpha+(getDisruptionLevel(ship)-0.25f));
            }
        }
